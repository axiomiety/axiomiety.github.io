...
For part 3 we'll want to set up a periodic task that removes any `BEPeer` that hasn't announced itself for a given torrent within the given interval. The ejection mechanism is based on a timer:


{% highlight golang %}
{% endhighlight %}

As this is essentially a web server we can leverage `http.ListenAndServe`:
