...
Using the same request as above, let's see what the tracker returns:
```
...
The response gives us a brief sense of the availability of the torrent. Connecting to a seed guarantees the required pieces are available.
...
type TrackerCache struct {
	...
	// keyed by info hash -> peer ID -> timestamp (seconds)
...
As this is essentially a web server we can leverage `http.ListenAndServe` after dispatching our background task:
