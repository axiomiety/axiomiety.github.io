...
Most libraries will have a pair of packages available - the actual library (the `.so` file - which stands for shared objects - similar to a `DLL` on Windows),compiled for your kernel/architecture, along with a `-dev` counterpart that contains the headers you'll need. Take `libssl` for instance:
...
Testing has become such an integral part of how I develop I feel naked without it. I have used a number of testing frameworks in probably as many languages, but when it came to C I drew a blank - I couldn't even name one. So instead I headed over to [Wikipedia](https://en.wikipedia.org/wiki/List_of_unit_testing_frameworks#C) and looked for something vaguely interesting. I came across [cmocka](https://cmocka.org/) which only seemed to be (1) actively maintained, (2) had a strong basis (`cmockery` from Google) and (3) minimal dependencies (again, that's something you need to worry about and why dependency management is so handy).
...
Let's make sure this is all wired up by replacing `throwaway/src/main.c` with a (slightly) modified example from libcurl's [excellent documentation](https://curl.se/libcurl/c/http-post.html):
...
❯ ./bin/main
{
  ...
  "headers": {
    ...
    "Host": "httpbin.org", 
    "X-Amzn-Trace-Id": "Root=1-61050b7a-6478e785736ff81a1a4d3fdc"
  }, 
  "json": null, 
  "origin": [redacted], 
...
Ran out of steam, will add something later ^_^
