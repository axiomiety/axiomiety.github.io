...
In [part 3]({% post_url 2024-09-25-go-bt-3 %}) we looked at leveraging trackers to obtain a list of peers serving a given torrent. In this post we'll look at connecting to peers to figure out which blocks are available.
...
The connection to a peer is both bidirectional and somewhat stateless. It's not simply a case of sending a message and listening for a response - communication from a peer can happen at any time, which is e.g. the case with with the `choke` message.

All messages are binary in essence - this is *not* a text-based protocol.
