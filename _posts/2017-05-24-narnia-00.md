...
Function "0x080484ca" not defined.
...
We first start by setting `EGG` to something we can easily recognise. The program loads the address of our environment variable (or more accurately the address of the contents) into `eax`. If we peak into what this address contains we see our payload. The next instruction (`call   eax`) essentially invokes our payload. Given it's doesn't represent anything right now it won't do much so let's fix that.
