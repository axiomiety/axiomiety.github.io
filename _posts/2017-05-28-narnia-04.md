...
This isn't the standard buffer overflow we're used to. We have to, somehow, change the value of `i` to be equal to 500. The code is using `snprintf` (writing to a sized buffer) so there won't be any overflow here. We do however have a trick up our sleeve.
